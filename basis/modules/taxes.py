...
class TaxesModule:
    def __init__(self, client, taxes_address: str):
        ...
        self._contract = self.client.web3.eth.contract(address=self.taxes_address, abi=self.taxes_abi)
    ...
    def get_tax_rate(self, token: str, user: str) -> int:
        ...
        return self._contract.functions.getTaxRate(checksum_token, checksum_user).call()

    def get_current_surge_tax(self, token: str) -> int:
        """Returns the current surge tax for a token."""
        checksum_token = Web3.to_checksum_address(token)
        return self._contract.functions.getCurrentSurgeTax(checksum_token).call()

    def get_available_surge_quota(self, token: str) -> int:
        """Returns the available surge quota for a token."""
        checksum_token = Web3.to_checksum_address(token)
        return self._contract.functions.availableSurgeQuota(checksum_token).call()

    def get_base_tax_rates(self) -> dict:
        """Returns the base tax rates for all token types."""
        stasis = self._contract.functions._taxRateStasis().call()
        stable = self._contract.functions._taxRateStable().call()
        default = self._contract.functions._taxRateDefault().call()
        prediction = self._contract.functions._taxRatePrediction().call()
        return {
            'stasis': stasis,
            'stable': stable,
            'default': default,
    ...
    def start_surge_tax(self, start_rate: int, end_rate: int, duration: int, token: str):
        ...
        """
        func = self._contract.functions.startSurgeTax(start_rate, end_rate, duration, Web3.to_checksum_address(token))
    ...
    def end_surge_tax(self, token: str):
        ...
        func = self._contract.functions.endSurgeTax(Web3.to_checksum_address(token))
        result = self.client.send_transaction(func)
        self._sync_tx(result['hash'])
        return result
    ...
    def add_dev_share(self, token: str, wallet: str, basis_points: int):
        ...
        """
        func = self._contract.functions.addDevShare(
    ...
    def remove_dev_share(self, token: str, wallet: str):
        ...
        func = self._contract.functions.removeDevShare(
